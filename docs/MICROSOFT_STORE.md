...
LoavyPlayer uses Tauri v2 and publishes a conventional Windows NSIS installer. This guide prepares the installer for the Microsoft Store's EXE/MSI submission path; it does not publish anything.
...
Tauri writes MSI bundles, when explicitly requested, to:

```text
src-tauri/target/release/bundle/msi/Loavy Player_<version>_x64_en-US.msi
```
...
If the project later switches to MSI, a local silent-install test can use:

```powershell
msiexec /i "Loavy Player_4.1.5_x64_en-US.msi" /quiet
```

Partner Center applies its default `/qn` switch to MSI packages. For the current EXE submission, enter `/S` as the installer parameter.
...
| Windows bundle target | NSIS, current-user install | Keep NSIS unless the Store submission is deliberately changed to MSI. |
...
The Tauri Publisher/Manufacturer value is also part of the installer's upgrade identity. Existing LoavyPlayer releases used `loavy`, so changing it requires an explicit registry migration; do not rename it only for display purposes. The verified legal publisher in Partner Center and the Authenticode certificate must still use the owner's real identity. Signing settings are intentionally absent because the certificate thumbprint, timestamp service, or custom signing command must come from the actual certificate provider. Unsigned local installers are for build validation only; sign the final release before submission.
...
- [ ] Host the signed installer at an immutable, versioned HTTPS URL. Do not replace the file at that URL after submission.
...
- [ ] Review [`PRIVACY.md`](../PRIVACY.md), replace its contact placeholder, publish it at a stable public HTTPS URL, and enter that URL in Partner Center. The policy covers local library scanning, optional metadata providers, downloads, and optional Room/Jam networking.
- [ ] In the privacy/data-access declaration, accurately disclose metadata lookups and peer-to-peer Room/Jam transfers even though LoavyPlayer has no central room service.
